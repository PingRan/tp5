<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:82:"D:\phpStudy\WWW\Property\public/../application/admin/view/default/owner\index.html";i:1534051801;s:82:"D:\phpStudy\WWW\Property\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<body>
	...
	<div class="cf">
		<a class="btn" href="<?php echo url('Owner/add'); ?>">新 增</a>
			...
			<tbody>
				<?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$owner): $mod = ($i % 2 );++$i;?>
					<tr>
						...
						<td><?php echo !empty($owner['bind'])?($owner['bind']==1?'业主':'亲属'):'租户'; ?></td>
                        <td><?php echo $owner['tel']; ?></td>
						<td><?php echo $owner['ID_Card']; ?></td>
						<td><?php echo !empty($owner['status'])?($owner['status']==1?'已验证':'未通过'):'待认证'; ?></td>
						<td><?php echo $owner['create_time']; ?></td>
						<td>
							<a title="查看" href="<?php echo url('show?id='.$owner['id']); ?>">查看</a>
							<a title="编辑" href="<?php echo url('edit?id='.$owner['id']); ?>">编辑</a>
