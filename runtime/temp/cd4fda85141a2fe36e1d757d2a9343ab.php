<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:81:"D:\phpStudy\WWW\Property\public/../application/admin/view/default/owner\show.html";i:1534054063;s:82:"D:\phpStudy\WWW\Property\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<p style="font-size: 15px">姓名:      <?php echo $list['name']; ?></p>
<p style="font-size: 15px">房号:      <?php echo $list['house']; ?></p>
<p style="font-size: 15px">关系:      <?php echo !empty($list['bind'])?$owner['bind']==1?'业主':'亲属':'租户'; ?></p>
<p style="font-size: 15px">账户:      <?php echo $list['user_id']; ?></p>
<p style="font-size: 15px">电话:      <?php echo $list['tel']; ?></p>
<p style="font-size: 15px">身份证:    <?php echo $list['ID_Card']; ?></p>
<p style="font-size: 15px">认证:      <?php echo !empty($list['status'])?($list['status']==1?'已认证':'未通过'):'待认证'; ?></p>
<p style="font-size: 15px">申请时间:   <?php echo $list['create_time']; ?></p>

<div class="main-title" style="margin-top: 20px;">
	<h3>已认证的账户</h3>
</div>
<?php if(!(empty($all) || (($all instanceof \think\Collection || $all instanceof \think\Paginator ) && $all->isEmpty()))): if(is_array($all) || $all instanceof \think\Collection || $all instanceof \think\Paginator): $i = 0; $__LIST__ = $all;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?>
  账号:  <?php echo $user['name']; ?>
  关系:<?php echo !empty($user['bind'])?$user['bind']==1?'业主':'亲属':'租户'; ?>
  联系电话  <?php echo $user['tel']; endforeach; endif; else: echo "" ;endif; endif; ?>
<div class="cf">
	<a class="btn" href="<?php echo url('Owner/index'); ?>">返回</a>
	<?php if($list['status'] != 1): ?>
	<a class="btn" href="<?php echo url('Owner/pass',array('id'=>$list['id'],'code'=>1)); ?>">通过认证</a>
	<?php else: ?>
	<a class="btn" href="<?php echo url('Owner/pass',array('id'=>$list['id'],'code'=>-1)); ?>">拒绝认证</a>
	<?php endif; ?>
</div>
